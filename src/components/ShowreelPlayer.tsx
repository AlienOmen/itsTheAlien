...
export function ShowreelPlayer({ src, poster, label = 'Play showreel' }: { src: string; poster: string; label?: string }) {
  ...
  return (
    <div className="relative aspect-video w-full overflow-hidden rounded-[40px] border border-line sm:rounded-[50px] md:rounded-[60px]">
      {playing ? (
        ...
        <button type="button" aria-label={label} onClick={() => setPlaying(true)} className="group h-full w-full">
